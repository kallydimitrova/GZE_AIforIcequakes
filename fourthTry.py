    ...
    model = Sequential(
        [
            ...
            keras.layers.Conv1D(filters=32, kernel_size=24, padding="same", strides=1, activation="relu",
                                kernel_regularizer=keras.regularizers.l2(0.02), kernel_initializer=initializers.RandomNormal()),
            keras.layers.BatchNormalization(),
            keras.layers.Dropout(rate=0.5),
            keras.layers.Conv1D(filters=16, kernel_size=12, padding="same", strides=1, activation="relu", kernel_initializer=initializers.RandomNormal(),
...
ensemble_predictions = np.zeros_like(predicted_labels)
...
# Perform ensemble decision (e.g., voting)
ensemble_predictions = np.round(ensemble_predictions)

# Evaluate the ensemble predictions
accuracy = accuracy_score(file_label_arrays, ensemble_predictions)
print("Ensemble Accuracy:", accuracy)

# Plot the ensemble predictions
for i in range(len(file_paths)):
    plot_file_2(data[i], ensemble_predictions[i], file_label_arrays[i])
